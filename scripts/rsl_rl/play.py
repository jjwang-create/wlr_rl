...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # Camera following setup for single environment (only active for keyboard mode)
    if env_cfg.scene.num_envs == 1 and args_cli.keyboard:
        camera_offset = torch.tensor([-4.0, 0.0, 1.0], device=env.unwrapped.device)  # Behind and above robot
    ...
    play_logger = PlayLogger(enabled=True)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # Update camera to follow robot only in keyboard-mode single-env
            if env_cfg.scene.num_envs == 1 and args_cli.keyboard:
            ...
            play_logger.log(env.unwrapped, env_id=0)
